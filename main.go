...
func main() {
	...
	mux.HandleFunc(pat.Put("/books/:isbn"), updateBooks(session))
	mux.HandleFunc(pat.Delete("/books/:isbn"), deleteBooks(session))
	http.ListenAndServe("localhost:8080", mux)
}
func deleteBooks(s *mgo.Session) func(w http.ResponseWriter, r *http.Request) {
...
func updateBooks(s *mgo.Session) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		session := s.Copy()
		defer session.Close()
...
func bookByISBN(s *mgo.Session) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			ErrorWithJSON(w, "Database error", http.StatusInternalServerError)
			log.Println("Falied find book: ", err)
			return
		}
